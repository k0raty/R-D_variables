...
def overview_regularizer(dataset,optimizer='adam',n_layers=4):
    ...
    overview=overview[overview['Train_accuracy']>0.60] #On récupère les modèles potables
    overview=overview[overview['Test_accuracy']>0.60]
    if len(overview)==0:
        print("Pas de modèles concluant , on garde tout les régularisateurs")
        return regularizers
    regularisateurs=list(overview['Regularizer'])
    fig1.savefig("Plots/Pré-paramétrage/Loss_regularizer")
    fig2.savefig("Plots/Pré-paramétrage/Train_regularizer")
    fig3.savefig("Plots/Pré-paramétrage/Test_regularizer")
    overview.to_excel("Excel/Pré-paramétrage/overview_regularizer.xlsx")
    overview.to_pickle("DataFrame/Pré-paramétrage/overview_regularizer.pkl")
    print("Tableurs enregistrés sous le nom de overview_regularizer")
