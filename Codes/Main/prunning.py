...
def smoothing_means(sheet,liste_temps):
    ...
    for l in tqdm(range(0,len(liste_temps.columns))): 
        ...
        max = q75+(1.5*intr_qr)
        min = q25-(1.5*intr_qr)
        filter = (liste_temps.iloc[:,l].dropna() >= min) & (liste_temps.iloc[:,l].dropna()<=max) #ce filtre garde l'information sur les index 
...
def prunning(sheet,sheet_2):
    ...
    fig.savefig("Plots/ecart_inter_quartile")
...
def smooth (sheet,liste_temps):
    ...
    overview(sheet.drop('index',axis=1),"Profil après nettoyage")
    return sheet,security
